...
def _count_critical(failures_map: dict[str, list[dict]]) -> int:
    return sum(
        1 for rows in failures_map.values()
        for r in rows if r.get("severity") == "CRITICAL"
    )
...
def _build_card(conn: sqlite3.Connection,
                company_id: str,
                failures_map: dict[str, list[dict]]) -> tuple[str, str]:
    ...
    lines.append(f"  P&L Coverage   : {pl_count} year(s)  [{year_span}]"
    ...
    if _table_exists(conn, "balancesheet"):
        ...
        bs_count = 0
    lines.append(f"  BS  Coverage   : {bs_count} year(s)"
    ...
    if _table_exists(conn, "profitandloss") and pl_years:
        ...
        if fin:
            ...
            lines += [
                ...
                f"  Revenue        : {rev:>12.2f} Cr" if rev is not None else "  Revenue        : —",
                f"  Net Profit     : {np_:>12.2f} Cr" if np_ is not None else "  Net Profit     : —",
                f"  OPM %          : {opm:>12.2f} %" if opm is not None else "  OPM %          : —",
            ]
        else:
...
def run_review() -> None:
    ...
    all_ids = [r[0] for r in conn.execute("SELECT company_id FROM companies").fetchall()]
