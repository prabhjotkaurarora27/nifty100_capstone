...
def _make_radar_chart(
    ...
    ax.set_rgrids([20, 40, 60, 80, 100],
                  ...
                  angle=22.5, fontsize=7, color="grey")
    ax.set_ylim(0, 100)
    ...
    ax.plot(ANGLES, ref_plot, linestyle="--", linewidth=1.5,
            color="#FF6B35", alpha=0.85, label=reference_label)
    ax.fill(ANGLES, ref_plot, color="#FF6B35", alpha=0.08)

    # Company polygon (filled)
    ax.plot(ANGLES, comp_plot, linewidth=2.0,
            color="#1B4F8A", alpha=0.9, label=company_id)
    ax.fill(ANGLES, comp_plot, color="#1B4F8A", alpha=0.20)

    # Title
    ax.set_title(
        f"{company_name}\n({company_id})",
        pad=18, fontsize=13, fontweight="bold", color="#1B4F8A",
    )
...
def _group_averages(df: pd.DataFrame, peer_map: Dict[str, str]) -> Dict[str, List[float]]:
    """Compute scaled average values per peer group and for Nifty 100."""
    # Nifty 100 average (all companies)
    nifty_avg = [
        np.nanmean([_scale(df.loc[cid, col] if cid in df.index else None, col, inv)
                    for cid in df.index])
    ...
    for grp, members in group_companies.items():
        avgs = []
        for _, col, inv in RADAR_AXES:
            vals = [_scale(df.loc[cid, col] if cid in df.index else None, col, inv)
                    for cid in members]
