...
def _load_all_data(db_path: Path) -> tuple:
    ...
    metric_col_names = ", ".join(f"fr.{col}" for col, _ in METRIC_COLS
                                  if col not in ("icr_label",))
    sql_ratios = f"""
...
def _write_peer_sheet(
    ...
    if not percentiles_df.empty:
        ...
        for _, row in grp_pct.iterrows():
            pct_pivot.setdefault(row["company_id"], {})[row["metric"]] = row["percentile_rank"]
    ...
    ws.merge_cells(start_row=2, start_column=1,
                   end_row=2, end_column=len(id_headers))
    ...
    ws.merge_cells(start_row=2, start_column=metric_start,
                   end_row=2, end_column=metric_start + len(metric_headers) - 1)
    ...
    ws.merge_cells(start_row=2, start_column=rank_start,
                   end_row=2, end_column=rank_start + len(rank_headers) - 1)
    ...
    numeric_data: Dict[str, List[float]] = {h: [] for h in metric_headers + rank_headers}
    ...
    for row_offset, cid in enumerate(company_ids):
        ...
        name_cell   = ws.cell(row=row_num, column=2,
                               value=ratio_row["company_name"].iloc[0]
                               if not ratio_row.empty else cid)
        year_cell   = ws.cell(row=row_num, column=3,
                               value=int(ratio_row["year"].iloc[0])
                               if not ratio_row.empty else None)
        ...
        for m_idx, (col_key, m_header) in enumerate(METRIC_COLS):
            ...
            raw = ratio_row[col_key].iloc[0] if not ratio_row.empty and col_key in ratio_row.columns else None
...
def export_peer_comparison_xlsx(
    ...
    wb.save(output_path)
    logger.info("✓ peer_comparison.xlsx saved → %s  (%d sheets)", output_path, len(group_names))
    return output_path
